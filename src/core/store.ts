...
export class SQLiteStore {
  ...
  private verifyAndFixSchema(): void {
    ...
    for (const [table, requiredIdxs] of Object.entries(requiredIndexes)) {
      try {
        const existingIndexes = this.db.prepare(`SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='${table}'`).all() as Array<{ name: string }>;
  ...
  getPeer(workspaceId: string, peerId: string): Peer | null {
    const row = this.db.prepare("SELECT * FROM peers WHERE id = ? AND workspace_id = ?").get(peerId, workspaceId) as any;
    if (!row) return null;
  ...
  getAllPeers(workspaceId: string): Peer[] {
    const rows = this.db.prepare("SELECT * FROM peers WHERE workspace_id = ?").all(workspaceId) as any[];
  ...
  getSession(workspaceId: string, sessionId: string): Session | null {
    const row = this.db.prepare("SELECT * FROM sessions WHERE id = ? AND workspace_id = ?").get(sessionId, workspaceId) as any;
    if (!row) return null;
    return { 
      id: row.id, 
  ...
  getAllSessions(workspaceId: string): Session[] {
    const rows = this.db.prepare("SELECT * FROM sessions WHERE workspace_id = ? ORDER BY created_at DESC").all(workspaceId) as any[];
    return rows.map((r) => ({ 
      id: r.id, 
  ...
  getSessionsByTag(workspaceId: string, tag: string, limit: number = 20): Session[] {
    const rows = this.db.prepare("SELECT * FROM sessions WHERE workspace_id = ? ORDER BY created_at DESC LIMIT ?").all(workspaceId, limit) as any[];
    return rows
      .map((r) => ({ 
        id: r.id, 
        workspaceId: r.workspace_id, 
  ...
  getConclusions(workspaceId: string, peerId: string, limit: number = 10): Conclusion[] {
    const rows = this.db.prepare("SELECT * FROM conclusions WHERE peer_id = ? AND workspace_id = ? ORDER BY created_at DESC LIMIT ?").all(peerId, workspaceId, limit) as any[];
  ...
  getAllConclusions(workspaceId: string, peerId?: string): Conclusion[] {
    ...
    const rows = this.db.prepare("SELECT * FROM conclusions WHERE workspace_id = ? ORDER BY created_at DESC").all(workspaceId) as any[];
    return rows.map((r) => ({ id: r.id, peerId: r.peer_id, type: r.type, content: r.content, premises: JSON.parse(r.premises || "[]"), confidence: r.confidence, createdAt: r.created_at, sourceSessionId: r.source_session_id, embedding: r.embedding ? JSON.parse(r.embedding) : undefined }));
  }
  ...
  getSummaries(workspaceId: string, peerId: string, limit: number = 10): Summary[] {
    const rows = this.db.prepare("SELECT * FROM summaries WHERE peer_id = ? AND workspace_id = ? ORDER BY created_at DESC LIMIT ?").all(peerId, workspaceId, limit) as any[];
  ...
  getPeerCard(workspaceId: string, peerId: string): PeerCard | null {
    const row = this.db.prepare("SELECT * FROM peer_cards WHERE peer_id = ? AND workspace_id = ?").get(peerId, workspaceId) as any;
    if (!row) return null;
  ...
  getObservations(workspaceId: string, peerId: string, limit: number = 100): Observation[] {
    const rows = this.db.prepare("SELECT * FROM observations WHERE peer_id = ? AND workspace_id = ? ORDER BY created_at DESC LIMIT ?").all(peerId, workspaceId, limit) as any[];
  ...
  getObservationsForPeer(workspaceId: string, observerPeerId: string, limit: number = 100): Observation[] {
    const rows = this.db.prepare("SELECT * FROM observations WHERE peer_id = ? AND workspace_id = ? AND about_peer_id IS NOT NULL ORDER BY created_at DESC LIMIT ?").all(observerPeerId, workspaceId, limit) as any[];
  ...
  getObservationsAboutPeer(workspaceId: string, targetPeerId: string, limit: number = 100): Observation[] {
    const rows = this.db.prepare("SELECT * FROM observations WHERE about_peer_id = ? AND workspace_id = ? ORDER BY created_at DESC LIMIT ?").all(targetPeerId, workspaceId, limit) as any[];
    return rows.map((r) => ({ id: r.id, peerId: r.peer_id, aboutPeerId: r.about_peer_id, sessionId: r.session_id, workspaceId: r.workspace_id, role: r.role, content: r.content, createdAt: r.created_at, embedding: r.embedding ? JSON.parse(r.embedding) : undefined, processed: r.processed === 1 }));
  }

  getUnprocessedObservations(workspaceId: string, peerId: string, limit: number = 50): Observation[] {
    const rows = this.db.prepare("SELECT * FROM observations WHERE peer_id = ? AND workspace_id = ? AND processed = 0 ORDER BY created_at ASC LIMIT ?").all(peerId, workspaceId, limit) as any[];
  ...
  searchSessions(workspaceId: string, query: string, limit: number = 10): Array<{ sessionId: string; createdAt: number; snippet: string; relevance: number }> {
    const rows = this.db.prepare(`
      ...
      LIMIT ?
    `).all(workspaceId, `%${query}%`, limit) as any[];
  ...
  exportAll(workspaceId: string): ExportData {
    ...
    const peerCards = this.db.prepare("SELECT * FROM peer_cards WHERE workspace_id = ?").all(workspaceId).map((r: any) => ({
