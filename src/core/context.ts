...
import type { PeerRepresentation, Conclusion, Summary, MemoryInsights } from "../shared.js";
...
export class ContextAssembler {
  ...
  async searchSimilar(
    ...
    topK = 5,
    minSimilarity = 0.0
  ): Promise<Array<Conclusion & { confidence: number }>> {
  ...
  getMemoryStats(workspaceId: string, peerId: string): MemoryStats {
    const rep = this.store.getRepresentation(workspaceId, peerId);
    if (!rep) return { conclusionCount: 0, summaryCount: 0, hasPeerCard: false, lastReasonedAt: null, topInterests: [], topTraits: [] };
  ...
  private buildContextString(rep: PeerRepresentation): string {
    const parts: string[] = [];
